...
class WP_To_Social_Pro_Log_Table extends WP_List_Table {
	...
	protected function bulk_actions( $which = '' ) {
		...
		if ( $which == 'top' ) {
			...
			<select name="action" size="1">
				<option value=""<?php selected( $this->get_action(), '' ); ?>><?php _e( 'Filter by Action', $this->base->plugin->name ); ?></option>
				<?php
			...
			<select name="profile_id" size="1">
				<option value=""<?php selected( $this->get_profile(), '' ); ?>><?php _e( 'Filter by Profile', $this->base->plugin->name ); ?></option>
				<?php
			...
			<select name="result" size="1">
				<option value=""<?php selected( $this->get_result(), '' ); ?>><?php _e( 'Filter by Result', $this->base->plugin->name ); ?></option>
				<option value="success"<?php selected( $this->get_result(), 'success' ); ?>><?php _e( 'Success', $this->base->plugin->name ); ?></option>
				<option value="test"<?php selected( $this->get_result(), 'test' ); ?>><?php _e( 'Test', $this->base->plugin->name ); ?></option>
				<option value="warning"<?php selected( $this->get_result(), 'warning' ); ?>><?php _e( 'Warning', $this->base->plugin->name ); ?></option>
				<option value="error"<?php selected( $this->get_result(), 'error' ); ?>><?php _e( 'Error', $this->base->plugin->name ); ?></option>
			</select>
		...
		<a href="<?php echo admin_url( 'admin.php?page=' . $this->base->plugin->name . '-log&bulk_action3=delete_all' ); ?>" class="button red">
	...
	public function search_box( $text, $input_id ) {
		...
		<p class="search-box">
			...
			<input type="search" id="<?php echo esc_attr( $input_id ); ?>" name="s" value="<?php _admin_search_query(); ?>" placeholder="<?php _e( 'Post ID or Title', $this->base->plugin->name ); ?>" />
	...
	public function get_columns() {
		...
		return array(
			...
			'post_id'				=> __( 'Post ID', $this->base->plugin->name ),
			'request_sent'			=> __( 'Request Sent', $this->base->plugin->name ),
			'action'				=> __( 'Action', $this->base->plugin->name ),
			'profile_name'			=> __( 'Profile', $this->base->plugin->name ),
			'status_text'			=> __( 'Status Text', $this->base->plugin->name ),
			'result'				=> __( 'Result', $this->base->plugin->name ),
			'result_message'		=> __( 'Response', $this->base->plugin->name ),
			'status_created_at'		=> sprintf( __( '%s: Status Created At', $this->base->plugin->name ), $this->base->plugin->account ),
			'status_due_at'			=> sprintf( __( '%s: Status Scheduled For', $this->base->plugin->name ), $this->base->plugin->account ),
		);
	...
	public function display_rows() {
        ...
        echo $this->base->get_class( 'log' )->build_log_table_output( $this->items, true );
