 ...
 */
function wp_to_socialpilot_activate( $network_wide ) {

    // Initialise Plugin
    $wp_to_socialpilot = WP_To_SocialPilot::get_instance();
    $wp_to_socialpilot->initialize();

    // Check if we are on a multisite install, activating network wide, or a single install
    if ( ! is_multisite() || ! $network_wide ) {
        // Single Site activation
        $wp_to_socialpilot->get_class( 'install' )->install();
    } else {
 ...
 */
function wp_to_socialpilot_activate_new_site( $blog_id ) {
